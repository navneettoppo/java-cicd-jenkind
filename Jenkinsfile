pipeline {
    ...
    stage('Pushing to ECR') {
      steps{
          docker.withRegistry("https://" + REPOSITORY_URI, "ecr:${AWS_DEFAULT_REGION}:") {        withCredentials([[          $class: 'AmazonWebServicesCredentialsBinding',          credentialsId: 'ecsagent',          region: 'us-east-1'        ]]) {          dockerImage.push()
            }
          }
        }
      }
